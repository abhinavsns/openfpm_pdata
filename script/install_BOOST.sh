...
#using gcc : 13 : g++-13 : <linkflags>"-Wl,-ld_classic" ;
...
if [ x"$platform" == x"osx" ]; then
    if [ x"$arch" == x"arm64" ]; then
      ...
      if [ x"$3" == x"" ]; then
          ...
          ./b2 -a -j $2 install --prefix=$1/BOOST address-model=64 architecture=x86 abi=sysv binary-format=mach-o toolset=$CC  -sNO_LZMA=1 -sNO_ZSTD=1
      fi
    fi
else
